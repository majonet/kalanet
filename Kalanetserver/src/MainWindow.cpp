...
void MainWindow::onUpdateProfile() {
    currentUser->setEmail(profileEmailEdit->text());
    currentUser->setPhone(profilePhoneEdit->text());
    currentUser->setAddress(profileAddressEdit->text());
...
void MainWindow::onCheckout() {
    ...
    for (auto it = customer->getCart().begin(); it != customer->getCart().end(); ++it) {
        ...
        if (product) {
            ...
            // Record transaction for seller if they are a customer
            Customer* sellerCustomer = dynamic_cast<Customer*>(seller);
            if (sellerCustomer) {
                Transaction sellerTrans = trans;
                sellerCustomer->addTransaction(sellerTrans);
            }
        }
    }
...
void MainWindow::onAddProduct() {
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        // Direct add - approved immediately
        product->setStatus(ProductStatus::APPROVED);
        dm->addProduct(product);

        // Add to admin's registered products
...
void MainWindow::onRegisterProduct() {
    // Same as add but goes to pending
    QDialog dialog(this);
...
void MainWindow::onApproveProduct() {
    ...
    int productId = pendingTable->item(row, 0)->text().toInt();
...
void MainWindow::onRejectProduct() {
    ...
    int productId = pendingTable->item(row, 0)->text().toInt();
    QString productName = pendingTable->item(row, 1)->text();
...
// My Products Tab Slots (for Customers)
void MainWindow::refreshMyProducts() {
    Customer* customer = dynamic_cast<Customer*>(currentUser);
    if (!customer) return;

    DataManager* dm = DataManager::getInstance();
    const QVector<int>& productIds = customer->getRegisteredProductIds();

    myProductsTable->setRowCount(productIds.size());

    for (int i = 0; i < productIds.size(); ++i) {
        Product* p = dm->getProduct(productIds[i]);
        if (p) {
...
void MainWindow::onRegisterNewProduct() {
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        // Add to customer's registered products
        Customer* customer = dynamic_cast<Customer*>(currentUser);
        if (customer) {
